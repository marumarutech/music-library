...
- `library/済み/` に移した曲も Web UI で一覧・編集
...
- ブラウザが `http://127.0.0.1:8765` を自動で開く
- 黒い窓（cmd）はサーバー実行中ずっと開いたまま（**Ctrl+C** で停止）
- 中身はリポジトリの `start.cmd`（株プロジェクトの `dev.cmd` と同じ方式）
...
`library/済み/` への移動は **エクスプローラーで手動**（ドラッグ＆ドロップ）。移動後は Web UI の「更新」で **済み** セクションに表示される。
