...
Pixel 向け音楽管理アプリを目指す個人プロジェクト。  
PC 上の **ローカル Web アプリ** または **CLI** から YouTube URL を貼って MP3 を取り込む。
...
```
music-library/
  core/             # 共通ロジック（取り込み・無音トリム・ライブラリ一覧）
  tools/ingest/     # CLI
  tools/web/        # ローカル Web UI
  library/          # ローカル音源（git 管理外）
  android/          # Pixel ネイティブアプリ（未実装）
```
...
2. **core** — ID3 タグ、プレイリスト
3. **android** — Jetpack Compose + Media3
